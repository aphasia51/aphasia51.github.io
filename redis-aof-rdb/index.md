...
![image-20220726112537499](https://cdn.jsdelivr.net/gh/aphasia51/images@main/blog/imagesimage-20220726112537499.png)
...
从以上两点可知，风险的发生都和写入磁盘的时机有关，这里Redis给我们提供了3中写回策略。

#### 写回策略

Redis写入AOF日志的流程:

![image-20220726150300832](https://cdn.jsdelivr.net/gh/aphasia51/images@main/blog/imagesaof_workframe.png)
...
上面我们一直再说将数据写入AOF日志，那么AOF日志就会越来越大，如果Redis需要重启，那么数据的回复也是一个比较耗时的过程(我们都知道AOF日志中记录的是其他格式的命令，所以文件过大，命令重放就会很慢)。所以就有了AOF重写机制

#### AOF重写机制

重写时候，扫描当前数据库中的所有键值对，将每一个键值对用一条命令记录到`新的AOF文件`，全部记录后再替换。很容易想到，如果之前我们对一个键值对中的值执行了更新操作，那么新的AOF文件中就只会记录最新的键值对对应的命令，之前的更新操作不会再记录，这样以来AOF文件就会压缩。但是仔细一想，重写的过程是比较耗时的，所以需要主进程fork一个子进程来**bgwriteaof**完成这个操作。
...
![image-20220726164856917](https://cdn.jsdelivr.net/gh/aphasia51/images@main/blog/imagescopy-on-write-on-use.png)
